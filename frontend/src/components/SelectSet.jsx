  ...
  const options = Object.entries(App.state.availableSets).reduce((acc, [setType, sets]) => {
    acc.push({
      ...
      items: sets.map(set => ({
        name: set.name,
        code: set.code,
        value: JSON.stringify({code: set.code, name: set.name})
      }))
    })
      ...
      renderValue={(valueProps) => {
        return (
          <div className="SelectSet__input-container" >
            <i className={`ss ss-${JSON.parse(value).code ? JSON.parse(value).code.toLowerCase() : ""}`} />
            <input className='SelectSet__input combo_natural' {...valueProps} />
          </div>
        )
      }}
      renderOption={(optionProps, optionData, snapshot, className) => {
        return (
          <button {...optionProps} className={className} type="button">
            <i className={`ss ss-${optionData.code.toLowerCase()}`} />
            {optionData.name}
