...
const boosterGeneratorMilanesapp = require("../boosterGeneratorMilanesapp");
...
const checkCode = (req, res, next) => {
  ...
  if (!req.params.code) {
    res.status(404).json({
      message: `Algo salío mal ${req.params.code}`
    });
  } else {
    next();
  }
...
gamesRouter
  ...
  */
  ...
  .get("/:code/booster", checkCode, (req, res) => {
    res.send(boosterGeneratorMilanesapp.makeBoosterFromRules(req.params.code));
  })
